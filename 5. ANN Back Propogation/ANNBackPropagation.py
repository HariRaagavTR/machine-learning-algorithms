...
class Tensor:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, self.__class__):
            if self.shape != other.shape:
                raise ArithmeticError(f"Shape mismatch for +: '{self.shape}' and '{other.shape}' ")
    ...
    def __matmul__(self, other):
        ...
        if not isinstance(other, self.__class__):
            raise TypeError(f"unsupported operand type(s) for matmul: '{self.__class__}' and '{type(other)}'")
        if self.shape[-1] != other.shape[-2]:
            raise ArithmeticError(f"Shape mismatch for matmul: '{self.shape}' and '{other.shape}' ")
        out = self.arr @ other.arr
        out_tensor = Tensor(out)
    ...
    def grad_add(self, gradients=None):
        ...
        Returns:
            ...
            Ex:
                ...
                Gradient to a and b
        """
        operand1, operand2 = self.history[1].arr, self.history[2].arr

        op1Grad = np.ones_like(operand1, dtype=np.float64)
        op2Grad = np.ones_like(operand2, dtype=np.float64)
        
        if gradients is not None:
            op1Grad = np.multiply(op1Grad, gradients)
            op2Grad = np.multiply(op2Grad, gradients)
        
        return (op1Grad, op2Grad)
    ...
    def grad_matmul(self, gradients=None):
        ...
        op1Grad = np.zeros_like(operand1)
        op2Grad = np.zeros_like(operand2)

        rows, cols = operand1.shape
        for col in range(cols):
            rowSum = np.sum(operand2[col, :])
            for row in range(rows):
                op1Grad[row, col] = rowSum

        rows, cols = operand2.shape
        for row in range(rows):
            colSum = np.sum(operand1[:, row])
            for col in range(cols):
                op2Grad[row, col] = colSum
    ...
    def backward(self, gradients=None):
        ...
        if self.history[0] == 'leaf':
            if self.requires_grad and gradients is not None:
                self.grad += gradients
        else:
            if self.history[0] == 'add':
                # Recursive call for add operation.
                operand_gradients = self.grad_add(gradients)
                self.history[1].backward(operand_gradients[0])
                self.history[2].backward(operand_gradients[1])
            else:
                # Recursive call for matmul operation.
                operand_gradients = self.grad_matmul(gradients)
                self.history[1].backward(operand_gradients[0])
                self.history[2].backward(operand_gradients[1])